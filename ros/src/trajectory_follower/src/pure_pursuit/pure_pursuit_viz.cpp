...
visualization_msgs::Marker displayNextWaypoint(geometry_msgs::Point position)
{
  ...
  marker.color.g = 0.0;
  marker.color.b = 1.0;
  marker.frame_locked = true; //retransformed into its frame every timestep
  return marker;
}

///\next_target display the next target by markers.
...
///\pp_locus display the locus of pure pursuit by markers.
